...
defmodule Google.Protobuf.Value do
  ...
  @type t :: %Value{
    kind: {:null_value, Google.Protobuf.NullValue.t} | {:number_value, number} | {:string_value, String.t} | {:bool_value, boolean} | {:struct_value, Google.Protobuf.Struct.t} | {:list_value, Google.Protobuf.ListValue.t}
  }
  ...
  def encode_to_iodata!(data) do
    alias Elixir.Pbuf.Encoder
    [
      Encoder.oneof_field(:null_value, data.kind, fn -> Encoder.enum_field(Google.Protobuf.NullValue, elem(data.kind, 1), <<8>>) end),
      Encoder.oneof_field(:number_value, data.kind, fn -> Encoder.field(:double, elem(data.kind, 1), <<17>>) end),
      Encoder.oneof_field(:string_value, data.kind, fn -> Encoder.field(:string, elem(data.kind, 1), <<26>>) end),
      Encoder.oneof_field(:bool_value, data.kind, fn -> Encoder.field(:bool, elem(data.kind, 1), <<32>>) end),
      Encoder.oneof_field(:struct_value, data.kind, fn -> Encoder.field(:struct, elem(data.kind, 1), <<42>>) end),
      Encoder.oneof_field(:list_value, data.kind, fn -> Encoder.field(:struct, elem(data.kind, 1), <<50>>) end),
    ]
  end
...
defmodule Google.Protobuf.NullValue do
  @moduledoc false
  @type t :: :NULL_VALUE | non_neg_integer
