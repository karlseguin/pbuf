defmodule OneOfZero do
  ...
  def encode_to_iodata!(data) do
    ...
    [
      Encoder.oneof_field(:a, data.choice, 0, fn v -> Encoder.field(:int32, v, <<8>>) end),
      Encoder.oneof_field(:b, data.choice, 0, fn v -> Encoder.field(:int32, v, <<16>>) end),
      Encoder.oneof_field(:value, data.json, 0, fn v -> 
case v do
        <<>> -> []
  ...
  def decode(acc, <<8, data::binary>>) do
    Decoder.oneof_field(:choice, 0, Decoder.field(:int32, :a, acc, data), nil)
  end
  
  def decode(acc, <<16, data::binary>>) do
    Decoder.oneof_field(:choice, 0, Decoder.field(:int32, :b, acc, data), nil)
  end
  
  def decode(acc, <<26, data::binary>>) do
    Decoder.oneof_field(:json, 0, Decoder.field(:bytes, :value, acc, data), fn v -> Elixir.Jason.decode!(v, []) end)
  end
